...
#define PRN 1000           
  ...
  if(*nmphi > 1){
	...
	for (k=0; k<l; k++){
	  ...
	  AA[(*n)*k - k*(k+1)/2+l]=corrfctvalue(phi_discrete[j],kappa,distcoords,(*cornr));
	}
    ...
    for (l=0; l<(*n); l++){
      for (k=0; k<l; k++){
	distcoords = sqrt( pow(coords1[k]-coords1[l],2) + pow(coords2[k]-coords2[l],2));
	AA[(*n)*k - k*(k+1)/2+l]=corrfctvalue(phi,kappa,distcoords,(*cornr));
      }
      AA[(*n)*l - l*(l+1)/2+l] = 1+ (*tausq); 
    }
    cholesky(AA,Q,(*n)); 
...
#define PRN 1000           
  ...
  if(*nmphi > 1){
	...
	for (k=0; k<l; k++){
	  distcoords = sqrt( pow(coords1[k]-coords1[l],2) + pow(coords2[k]-coords2[l],2));
	  AA[(*n)*k - k*(k+1)/2+l]=corrfctvalue(phi_discrete[j],kappa,distcoords,(*cornr)) + DDvbetaDD[l*(*n)+k];
	}
    ...
    for (l=0; l<(*n); l++){
      for (k=0; k<l; k++){
	distcoords = sqrt( pow(coords1[k]-coords1[l],2) + pow(coords2[k]-coords2[l],2));
	AA[(*n)*k - k*(k+1)/2+l]=corrfctvalue(phi,kappa,distcoords,(*cornr)) + DDvbetaDD[l*(*n)+k];
      }
      AA[(*n)*l - l*(l+1)/2+l]= 1 + DDvbetaDD[l*(*n)+l] + (*tausq);  
    }
    cholesky(AA,Q,(*n)); 
...
void newmcmcrun5(Integer *n, Real *data, Real *units, Real *meanS, Real *DDvbetaDD, Integer *cornr, Real *kappa, Real *tausq, Real *thetaS,
	      Real *coords1, Real *coords2, Real *coords3, Real *scale, Real *phiscale,Real *Htrunc, Integer *niter, Integer *subsample, 
	      Integer *burn_in, Integer *messages, Real *ss_sigma, Integer *df, Real *phiprior,  Real *phi_discrete, Integer *nmphi, Real *SS, 
	      Real *phisamples, Real *acc_rate, Real *acc_rate_phi){
...
#define PRN 1000           
  Integer i, j, jj, l, k, acc, acc_phi, dim2, itr, jstep, jphi ; 
  Real logfprop, logp_zprop,logf, logp_z, logq, logqprop, ss5, ss5prop, phi, phiprop, phistep, logp_phi, logp_phiprop, temp_phi ;   
  Real distcoords, Sdistcoords, Tdistcoords;
  Doublearray z, zprop, S, Sprop, gradz, gradzprop, temp;
  Doublearray Q, Qprop, AA;
  Doublearray BB[2001];
  ...
  if(*nmphi > 1){
    ...
    for (j=0;j<(*nmphi);j++){  
      ...
      for (l=0; l<(*n); l++){
	for (k=0; k<l; k++){
		//what to do 4/30/2013
	  Sdistcoords = sqrt( pow(coords1[k]-coords1[l],2) + pow(coords2[k]-coords2[l],2));
	  Tdistcoords = abs(coords3[k]-coords3[l]);
	  distcoords = Sdistcoords*(*thetaS) + Tdistcoords;
	  AA[(*n)*k - k*(k+1)/2+l]=corrfctvalue(phi_discrete[j],kappa,distcoords,(*cornr)) + DDvbetaDD[l*(*n)+k];
	}
	AA[(*n)*l - l*(l+1)/2+l]= 1 + DDvbetaDD[l*(*n)+l] + (*tausq);  
      }
