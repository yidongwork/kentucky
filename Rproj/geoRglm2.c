...
Real corrfct(Real phi, Real kappa, Real h , Integer cornr)
{
...
void newmcmcrun5(Integer *n, Real *data, Real *units, Real *meanS, Real *DDvbetaDD, Integer *cornr, Real *kappa, Real *tausq, Real *thetaS, //thetaS is the coefficieint of spatial 
	      Real *coords1, Real *coords2, Real *coords3, Real *scale, Real *phiscale,Real *Htrunc, Integer *niter, Integer *subsample, // coords3 is for time scale
	      Integer *burn_in, Integer *messages, Real *ss_sigma, Integer *df, Real *phiprior,  Real *phi_discrete, Integer *nmphi, Real *SS, 
	      Real *phisamples, Real *acc_rate, Real *acc_rate_phi){
...
#define PRN 1000           
  Integer i, j, jj, l, k, acc, acc_phi, dim2, itr, jstep, jphi ; 
  Real logfprop, logp_zprop,logf, logp_z, logq, logqprop, ss5, ss5prop, phi, phiprop, phistep, logp_phi, logp_phiprop, temp_phi ;   
  Real distcoords, Sdistcoords, Tdistcoords;//Tdistcoords is time interval
  Doublearray z, zprop, S, Sprop, gradz, gradzprop, temp;
  Doublearray Q, Qprop, AA;
  Doublearray BB[2001];
  ...
  if(*nmphi > 1){
	...
	for (k=0; k<l; k++){
	  ...
	  //right now just add them up with coefficient thetas
